...
def update_output(tab, n_clicks_begin, n_clicks_begin_2, n_clicks_after, source_amount, request_amount, buff_amount,
    ...
    if tab == 'tab1':
        if n_clicks_begin is None:
            return [html.H5(children='Временная диаграмма событий'),
            ...
            if n_clicks_begin == 1:
                ...
                global smo
                smo = System(int(source_amount), int(request_amount), int(buff_amount), int(device_amount))
        ...
        if n_clicks_begin_2 is None:
            ...
            if n_clicks_begin_2 == 1:
                smo = Sysrem(int(source_amount), int(request_amount), int(buff_amount), int(device_amount))
                smo.all_iteration()
                print("я здесь был")
                return [html.H5(children='Источники'),
